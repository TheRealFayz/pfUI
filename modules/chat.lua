pfUI:RegisterModule("chat", "vanilla:tbc", function ()
  ...
  if C.chat.global.tabmouse == "1" then
    ...
    pfUI.chat.mouseovertab:SetScript("OnUpdate", function()
      -- throttle (guarded: pfUI.throttle may be missing/load later)
      if ( this.tick or .1) > GetTime() then return else this.tick = GetTime() + ((pfUI.throttle and pfUI.throttle:Get("chat_tab")) or 0.1) end  -- Default: Normal (10 FPS)
